...
### Changed
- 共通 `DateInput` を `YYYYMMDD` の半角数字8桁入力へ変更し、保存時に `YYYY-MM-DD` 相当の日付へ変換するように整理。
- `/account` の生年月日と `/renewals` の交付日、有効期限日、許可日、確認日、更新申請期間、銃本体の日付入力を半角数字8桁入力に統一。
- 不正な日付入力（桁不足、半角数字以外、存在しない日付）は保存せず、Server Action 側でフィールド単位エラーを返すように変更。
- `/account` の電話番号を、全角数字は半角へ変換し、ハイフン・空白を除去して数字のみで保存するように変更。
- 銃砲所持許可の有効期限日自動計算が、半角数字8桁の許可日入力でも維持されるように調整。
- 日付入力欄の和暦確認表示を、正しい8桁日付入力後だけ「入力内容」として表示するように整理。
- 未入力時、入力途中、不正な8桁日付では和暦確認表示を出さず、保存済み日付が固定例と混ざって見えないように変更。
- `/renewals` の銃砲所持許可では有効期限日を手入力させず、許可日と利用者本人の生年月日から自動計算した結果を表示・保存するように変更。
- `/renewals` の銃砲所持許可フォームで、許可日と交付日を別枠入力にし、有効期限日を許可日基準で自動計算するように変更。
- `/renewals` 新規登録フォームの種別切替時に、銃砲所持許可の許可日・交付日・有効期限日表示が基本項目として切り替わるよう表示漏れを追加修正。
- `/renewals` の狩猟免許表示名を「狩猟免許（狩猟免状）」へ変更し、有効期限日入力欄を削除して、交付日の年 + 3年の9月14日を自動表示・保存するように変更。
- `/renewals` の種別切替時に、銃砲所持許可と狩猟免許（狩猟免状）で説明文・入力項目・有効期限表示が切り替わるよう整理。

### Verified
- `nodebrew exec v20.20.2 -- npm run typecheck` 成功。
- `nodebrew exec v20.20.2 -- npm run build` 成功。
- `/account` と `/renewals` のHTTP 200表示を確認。

## 2026-05-12

### Added
- Vercel + Neon 第1段階本番テスト向けに、主要ページの手動CRUD確認用チェックリスト `PRODUCTION_MANUAL_TEST.md` を追加。
  - 画像アップロードは今回の確認対象外として整理。
  - `/` / `/renewals` / `/ammo` / `/reports` / `/account` の表示と、画像アップロード以外のCRUD確認項目を表形式で整理。
  - 実際の本番CRUD確認完了は未記録。確認後に `TODO.md` / `CHANGELOG.md` へ別途反映する。

### Changed
- 銃砲所持許可の有効期限日を、利用者の生年月日と許可日から「許可日から見て3回目に来る誕生日」で自動計算できるように変更。
- `/renewals` の銃砲所持許可証情報の日付表示を、和暦メイン・西暦括弧付きに寄せて表示するように変更。
- `/renewals` の新規登録フォームと銃砲所持許可証情報の編集フォームから「進捗」選択欄を外し、新規登録時は内部的に `ACTIVE` として保存するように変更。
- `/renewals` の登録済みカードに残っていた「管理状態」表示も外し、進捗欄と誤認される表示漏れを解消。
- `/renewals` の新規登録フォームに残っていた内部用 `status` hidden input も外し、Server Action側の既定値 `ACTIVE` で保存する形に整理。
- `PRODUCTION_MANUAL_TEST.md` に、本番トップページ表示と `/renewals` 表示がOKであること、`/renewals` 新規登録確認は今回の改善後に再確認することを追記。
...
- 活動記録追加フォームの狩猟記録 / 射撃記録切替をタブUIに変更し、狩猟は薄い緑、射撃は薄い青のフォーム背景で表示するように変更。
- 保存・登録・更新・追加系の主要CTAを `#34C759` ベースの明るい緑と太字白文字に統一し、削除・キャンセル・タブ切替は既存色を維持。
...
- `/reports` のHTML出力でタブUI、狩猟記録 / 射撃記録、狩猟記録側の薄い緑背景クラスを確認。
- `/renewals` / `/ammo` / `/reports` / `/account` のHTTP 200表示を確認。

### Production Test Notes
- Neon のDBパスワードを再発行し、Vercel Production の `DATABASE_URL` を新しい Neon 接続文字列へ差し替え。
- Vercel Production を Redeploy。
- 本番URLで `activityType` カラム不足エラーが出たため、Neon 本番DBへ `prisma:push` を実行。
- Neon 本番DBが現在の Prisma schema と同期され、本番URLでエラー解消を確認。
- 今回の記録更新では、schema変更・実装変更・追加の本番DB操作は行っていない。
