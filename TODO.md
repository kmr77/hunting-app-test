...
- [x] 日付入力UIを見直し、半角数字8桁（YYYYMMDD）入力へ変更する
...
- [x] 銃砲所持許可の有効期限日を許可日と生年月日から自動計算し、和暦メイン表示にする
- [x] `/renewals` の新規登録・編集フォームから進捗項目を外し、表示漏れになっていた管理状態表示と新規登録側の内部hidden入力も削除する
- [x] `/account` の電話番号入力をハイフンなし半角数字形式に統一し、保存時に数字のみへ正規化する
- [x] `/account` と `/renewals` の日付入力を半角数字8桁で受け付け、保存時に日付型へ変換する
- [x] 日付入力欄の和暦確認表示を、正しい8桁日付入力後だけ表示する
- [x] 銃砲所持許可の有効期限日入力欄を削除し、許可日と生年月日からの自動計算表示に変更する
- [x] 銃砲所持許可の許可日・交付日を別枠入力にし、有効期限日を許可日基準で自動計算する
- [x] `/renewals` 新規登録フォームの種別切替で銃砲所持許可の許可日・交付日・有効期限日表示が切り替わらない表示漏れを追加修正する
- [x] 狩猟免許（狩猟免状）の有効期限日入力欄を削除し、交付日基準の9月14日満了を自動計算表示に変更する
- [x] `/renewals` の種別切替表示を銃砲所持許可・狩猟免許（狩猟免状）ごとに整理する
...
- [x] 本番手動CRUD確認用チェックリストを作成する
- [ ] 一般公開MVPの正式構成を決定する（現時点では未確定）
- [x] 本番 PostgreSQL を選定・セットアップする（第1段階は Neon）
- [ ] 本番環境用 `.env` テンプレートを作成
...
- [x] 本番環境へコードをデプロイ
- [x] 本番環境でデータベース初期化（`prisma:push`）
- [ ] 本番環境でデモユーザー作成（`ensure-demo-gun-license.mjs` 実行）

### デプロイ後動作確認

- [ ] 本番 URL でトップページが表示される
- [x] 本番 DB が正しく接続されているか（本番URLの `activityType` カラム不足エラー解消を確認）
