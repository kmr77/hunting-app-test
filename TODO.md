...
- [x] フォームの field 単位エラー表示を追加する
- [x] 登録済み所持銃情報の銃本体削除導線を追加する
- [x] 更新チェッカーの自動計算ルールを北海道/全国展開向けに `validation_rules` へ寄せる
- [x] 許可証記載項目のフォーム単位バリデーションと全角/半角正規化ルールを拡張する
- [x] 日付入力UIを見直し、type="date" に統一してカレンダー入力可能にする

## 優先度: 中

- [x] 活動記録で狩猟記録/射撃記録を分けるDB項目・保存処理・UI切替をローカル実装する
- [x] 活動記録追加フォームの狩猟記録/射撃記録切替をタブUI化し、選択中の背景色を記録種別ごとに変更する
- [x] 保存・登録・更新・追加系CTAを `#34C759` ベースの明るい緑と太字白文字に統一する
- [x] デモユーザー前提を本番認証導入へ移行しやすい形に整理する
- [x] 共通フォーム UI を整理する
- [x] 成功 / 失敗メッセージの表示ルールをさらに揃える
- [x] 一覧の空状態、件数表示、補助文言を微調整する
- [x] 許可証画像の本番保存先を検討する
- [x] 自分専用本番化前チェックリストを作成する
- [x] Vercel本番化構成を Neon / Supabase / Vercel Blob の3案で比較し、DEPLOY_OPTIONS.md に整理する
...
- [x] デプロイ先環境を選定する（第1段階の自分専用本番テストは Vercel + Neon 無料枠）
- [x] 第1段階は Vercel + Neon 無料枠で本体・DB接続テストを行う
- [x] 画像アップロードは第1段階の本番確認対象外にする
- [ ] 一般公開MVPの正式構成を決定する（現時点では未確定）
- [ ] 本番 PostgreSQL を選定・セットアップする（Neon / Supabase / RDS / レンタルサーバー等）
- [ ] 本番環境用 `.env` テンプレートを作成
- [ ] 本番環境への初期接続テスト（`npm run db:check` 確認）

### デプロイ前最終確認

- [ ] ローカル環境で全検証コマンド成功確認（`lint`, `typecheck`, `build`）
- [ ] ローカル `npm run dev` で全5ページ HTTP 200 確認
- [ ] 各ページの CRUD 操作を手動テスト
- [ ] 画像アップロード以外の CRUD 操作を本番相当環境で手動テスト

### 本番デプロイ

- [ ] 本番環境へコードをデプロイ
- [ ] 本番環境でデータベース初期化（`prisma:push`）
- [ ] 本番環境でデモユーザー作成（`ensure-demo-gun-license.mjs` 実行）

### デプロイ後動作確認

- [ ] 本番 URL でトップページが表示される
- [ ] 本番 DB が正しく接続されているか（ダッシュボード数字表示確認）
- [ ] 各ページの CRUD 操作を本番環境で手動テスト
- [ ] 第1段階後に、許可証画像のアップロード・削除を本番環境で手動テスト
- [ ] 定期バックアップ設定（DB・ファイル）

## 優先度: 低

- [ ] UI の細部調整
- [ ] 標準date inputの選択表示・カレンダーUIの見た目改善を検討する
- [ ] ソートや検索などの操作性改善
- [ ] 利用履歴や監査ログの検討
